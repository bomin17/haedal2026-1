...
import com.example.haedal.dto.PostResponseDto;
...
public class PostService {
    ...
    public List<PostResponseDto> getPostsByUser(Long targetUserId) {
        ...
        return posts.stream().map(post -> convertPostToDto(targetUser, post)).toList(); //read only
    }


    private PostResponseDto convertPostToDto(User currentUser, Post post) { //post entity를 가지고 dto로 반환해주는 메소드
